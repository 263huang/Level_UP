...
def api_get_roadmap():
    ...
    c = conn.cursor()
    c.execute('SELECT title FROM targets WHERE id=?', (target_id,))
    row = c.fetchone()
    target_title = row[0] if row else '未命名目标'
    conn.close()
    roadmap = get_roadmap(user_id, target_id)
    return jsonify({'roadmap': roadmap, 'target_title': target_title})
